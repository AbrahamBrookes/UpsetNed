...
func Physics_Update(_delta: float):
	...
	# Update animation
	state_machine.anim_tree.set("parameters/Locomotion/Locomote/blend_position", Vector2(-input_direction.x, input_direction.y))
	...
	if multiplayer.is_server():
		...
		if input_direction != Vector2.ZERO:
			...
			mesh_rotation.y = camera_rotation.y + PI # invert the rotation because I don't know why
			mesh.rotation = mesh_rotation # this gets replicated back to the client
		...
		# transform the velocity to be facing from the mesh, as it has been rotated
		player_character.velocity = mesh.global_transform.basis * world_direction
		player_character.move_and_slide() # position gets replicated back to the client
